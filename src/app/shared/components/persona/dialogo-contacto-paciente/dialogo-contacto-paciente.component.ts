...
import { BuscadorPersonasGenericoComponent } from "../buscador-personas-generico/buscador-personas-generico.component";
import { TPO_FND_CT } from "../../catalogos/catalogos.const";
import { CT_TIPO_RELACIONES } from "src/app/shared/interfaces/persona-const";
@Component({
  ...
  asignaDataResidenciaContacto(accion?: string, contactoPersona?: any) {
    ...
    this.pacienteService.contactoSelected$.subscribe({
      next: (p: any) => {
        ...
        // else {
        //   Swal('Error', 'No es posible obtener datos de contacto', 'error');
        // }
      },
      error: (err: any) => {
        Swal.fire("error", err, "error");
